...
public class CoastController
{
    ...
    public MyCharacterController FindCharacterOnCoast(int role) {
        // role: 0->priest, 1->devil
        for (int i = 0; i < passengerPlaner.Length; ++i) {
            if (passengerPlaner[i] != null && passengerPlaner[i].getType() == role) {
                return passengerPlaner[i];
            }
        }
        Debug.Log("cannot find passenger " + role);
        return null;
    }
